...
export class ModelUser {
    ...
    static async getOneUser(parameters) {
        ...
        if (findOneUser === null) return new ResourceNotFoundError('not found user.');
        return findOneUser;
    }
    ...
    static async createUser(input) {
        ...
        try {
            ...
            return new QueryErrors(`Error in the query detail generate salt: ${error}`);
        }
        let hash;
        try {
            hash = bcrypt.hashSync(input.password, salt, 15, HASH_KEY_USER);
        } catch (error) {
            return new QueryErrors(`Error in the query detail create hash: ${error}`);
        }
        input.password = hash;
        let newUser;
        try {
            ...
            return new QueryErrors(`Error in the query detail schema user: ${error}`);
        }
        let saveNewUser;
        try {
            saveNewUser = await newUser.save();
        } catch (error) {
            return new QueryErrors(`Error in the query detail save user: ${error}`);
        }
        return saveNewUser;
    }
    ...
    static async deleteUser(username) {
        if (!username) return new ValidationError('the information query parameters were not sent.');
        try {
    ...
    static async updateUser(user, input) {
        if (!user) return new ValidationError('the information query parameters were not sent.');
        if (input.password) {
            let salt;
            try {
                salt = bcrypt.genSaltSync(15);
        ...
        try {
            ...
            return new QueryErrors(error, `Error in the query detail: ${error}`);
        }
        if (input.changeData) {
            ...
            try {
                ...
                return new QueryErrors(error, `Error in update change history: ${error}`);
            }
